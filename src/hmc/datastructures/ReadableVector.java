...
public interface ReadableVector {
    ...
    class Utils {
        ...
        public static void setParameter(double[] value, RealParameter parameter) {
            for (int j = 0, dim = value.length; j < dim; ++j) {
                parameter.setValue(j, value[j]);
            }
            //parameter.fireParameterChangedEvent();
        }

        public static void setParameter(ReadableVector position, RealParameter parameter) {
            for (int j = 0, dim = position.getDim(); j < dim; ++j) {
                parameter.setValue(j, position.get(j));
            }
            //parameter.fireParameterChangedEvent();
        }


        public static void setParameter(WrappedVector position, RealParameter parameter) {
...
//                par[j] = pos[posOffset + j];
                parameter.setValue(j, pos[posOffset + j]);
            }
//             parameter.fireParameterChangedEvent();
        }        
