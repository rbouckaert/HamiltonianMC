...
//        }
        ...
        if (!Arrays.equals(destination.getParameter().getDoubleValues(), parameter.getDoubleValues())){
            throw new RuntimeException("Unequal parameter values");
        }

        this.likelihood = new Distribution() {

            @Override
            public double calculateLogP() {

                double likelihood = source.getLikelihood().calculateLogP();

                if (beta != 1.0) {
                    likelihood = blend(likelihood, destination.getLikelihood().calculateLogP(), beta);
                }
