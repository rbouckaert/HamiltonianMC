...
//                AdaptableCovariance adaptableCovariance = new AdaptableCovariance.WithSubsampling(gradient.getDimension(), 1000);
    ...
    class NoPreconditioning implements MassPreconditioner {
        ...
        public WrappedVector drawInitialMomentum() {
            ...
            for (int i = 0; i < dim; ++i) {
                momentum[i] = Randomizer.nextGaussian();
            }
    ...
    abstract class AbstractMassPreconditioning extends CalculationNode implements MassPreconditioner {
        final protected int dim;
        final protected Transform transform;
        protected RealParameter inverseMass;
        ...
        protected void setInverseMassFromArray(double[] inverseMassArray) {
            for (int i = 0; i < inverseMassArray.length; i++) {
                inverseMass.setValue(i, inverseMassArray[i]);
            }
        }
...
//        @Override
//        protected void handleVariableChangedEvent(Variable variable, int index, RealParameter.ChangeType type) {
//
//        }
    ...
    abstract class DiagonalPreconditioning extends AbstractMassPreconditioning {
        ...
        protected DiagonalPreconditioning(int dim, Transform transform) {
            ...
            this.inverseMass = new RealParameter();
            this.inverseMass.setID(MASSNAME);
            this.inverseMass.setDimension(dim);
            initializeMass();
//            addVariable(inverseMass);
        ...
        public WrappedVector drawInitialMomentum() {
            ...
            for (int i = 0; i < dim; i++) {
                momentum[i] = Randomizer.nextGaussian() * Math.sqrt(1.0 / inverseMass.getArrayValue(i));
            }
        ...
        @Override
        public double getVelocity(int i, ReadableVector momentum) {
            return momentum.get(i) * inverseMass.getArrayValue(i);
        }
        ...
        public ReadableVector doCollision(int[] indices, ReadableVector momentum) {
            ...
            final double updatedMomentum1 = ((inverseMass.getArrayValue(index2) - inverseMass.getArrayValue(index1)) * momentum.get(index1)
                    + 2.0 * inverseMass.getArrayValue(index2) * momentum.get(index2)) / (inverseMass.getArrayValue(index1) + inverseMass.getArrayValue(index2));
            final double updatedMomentum2 = ((inverseMass.getArrayValue(index1) - inverseMass.getArrayValue(index2)) * momentum.get(index2)
                    + 2.0 * inverseMass.getArrayValue(index1) * momentum.get(index1)) / (inverseMass.getArrayValue(index1) + inverseMass.getArrayValue(index2));
    ...
    class PriorPreconditioner extends DiagonalPreconditioning {
        ...
        protected void computeInverseMass() {
            ...
            for (int i = 0; i < priorDistribution.getDimension(); i++){
                ...
                inverseMass.setValue(i, stDev * stDev);
            }
    ...
    class DiagonalHessianPreconditioning extends DiagonalPreconditioning {
        ...
        final protected HessianWrtParameterProvider hessian;
        final private RealParameter lowerBound;
        final private RealParameter upperBound;

        DiagonalHessianPreconditioning(HessianWrtParameterProvider hessian,
                                       Transform transform,
                                       int memorySize,
                                       RealParameter lowerBound,
                                       RealParameter upperBound) {
        ...
        protected void computeInverseMass() {
            ...
            if (transform != null) {
                ...
                double[] untransformedValues = hessian.getParameter().getDoubleValues();
        ...
        public static double[] boundMassInverse(double[] diagonalHessian, RealParameter lowerBound, RealParameter upperBound,
            ...
            for (int i = 0; i < dim; i++) {
                ...
                if (boundedMassInverse[i] < lowerBound.getArrayValue(0)) {
                    boundedMassInverse[i] = lowerBound.getArrayValue(0);
                } else if (boundedMassInverse[i] > upperBound.getArrayValue(0)) {
                    boundedMassInverse[i] = upperBound.getArrayValue(0);
                }
            }
    ...
    class AdaptiveDiagonalPreconditioning extends DiagonalPreconditioning {
        ...
        private void setInitialMass() {
            double[] values = gradient.getParameter().getDoubleValues();
            double[] storedValues = values.clone();
            for (int i = 0; i < dim; i++) {
                gradient.getParameter().setValue(i, values[i] + MachineAccuracy.SQRT_SQRT_EPSILON);
            }
            ...
            for (int i = 0; i < dim; i++) {
                gradient.getParameter().setValue(i, values[i]);
            }
            //gradient.getParameter().fireParameterChangedEvent();
            ...
            for (int i = 0; i < dim; i++) {
                ...
                gradient.getParameter().setValue(i, storedValues[i]);
            }
            //gradient.getParameter().fireParameterChangedEvent();
            fillZeros(values);
        ...
        public WrappedVector getMass() {
            double[] mass = new double[dim];
            for (int i = 0; i < dim; i++) {
                mass[i] = 1 / inverseMass.getArrayValue(i);
            }
            return new WrappedVector.Raw(mass);
        }
    }
    ...
    class FullHessianPreconditioning extends HessianBased {
        ...
        FullHessianPreconditioning(HessianWrtParameterProvider hessian, Transform transform, int dim) {
            ...
            // this.inverseMass = new RealParameter.Default(MASSNAME, dim * dim);
            this.inverseMass = new RealParameter();
            this.inverseMass.setID(MASSNAME);
            this.inverseMass.setDimension(dim * dim);
            // addVariable(inverseMass);
        }
        ...
        private double[] computeInverseMass(WrappedMatrix.ArrayOfArray hessianMatrix, GradientWrtParameterProvider gradientProvider, PDTransformMatrix pdTransformMatrix) {
            ...
            if (transform != null) { // TODO: change 0 matrix into general TransformationHessian
                transformedHessian = transform.updateHessianLogDensity(transformedHessian, new double[dim][dim], gradientProvider.getGradientLogDensity(), gradientProvider.getParameter().getDoubleValues(), 0, dim);
            }
        ...
        public WrappedVector drawInitialMomentum() {
            ...
            MultivariateNormalDistribution mvn = new MultivariateNormalDistribution(
                    new double[dim], toArray(inverseMass.getDoubleValues(), dim, dim)
            );
        ...
        public double getVelocity(int i, ReadableVector momentum) {
            ...
            for (int j = 0; j < dim; ++j) {
                velocity += inverseMass.getArrayValue(i * dim + j) * momentum.get(j);
            }
...
//            double[][] numericHessian = NumericalDerivative.getNumericalHessian(numeric1, gradientProvider.getParameter().getDoubleValues());
        ...
        protected MultivariateFunction numeric1 = new MultivariateFunction() {
            ...
            public double evaluate(double[] argument) {
                ...
                for (int i = 0; i < argument.length; ++i) {
                    gradientProvider.getParameter().setValue(i, argument[i]);
                }
