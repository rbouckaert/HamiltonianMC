...
import hmc.operator.GradientProvider;
import hmc.operator.HessianProvider;
...
public class MultivariateNormalDistribution implements MultivariateDistribution, // GaussianProcessRandomGenerator,
    ...
    public static double calculatePrecisionMatrixLogDeterminate(double[][] precision) {
        try {
        	LUPDecomposition LUP = new LUPDecomposition(new Matrix(precision));
        	return LUP.logDeterminant();
    ...
    public static double[] nextMultivariateNormalViaBackSolvePrecision(double[] mean, double[] precision) {
        ...
        for (int i = 0; i < dim; ++i) {
            epsilon[i] = Randomizer.nextGaussian();
        }
